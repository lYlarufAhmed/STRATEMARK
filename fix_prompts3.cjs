...
const startPrompt = code.indexOf('const prompt = `You are the core intelligence engine for STRATEMARK');
const endPrompt = code.indexOf('DO NOT wrap it in \\`\\`\\`json.`;') + 'DO NOT wrap it in \\`\\`\\`json.`;'.length;
...
if (startPrompt !== -1 && endPrompt !== -1) {
  code = code.substring(0, startPrompt) + newBoardPrompt + code.substring(endPrompt);
}
